...
For the Housekeeping single current-task fast path, source and deterministic contract checks may support only the structural claim that eligible requests require fewer reference loads and model/tool round trips. Measured wall-clock latency remains unverified unless a separately approved evaluation satisfies the paired performance contract above; do not convert the structural claim into a universal seconds-level SLA.

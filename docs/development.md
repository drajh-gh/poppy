...
3. Keep product content project-agnostic, synthetic, machine-path-free, and confined to declared skills plus optional bounded hooks. Housekeeping stays stateless; Scribe checkpoints stay conversation-bound and never emit a raw checkpoint payload in assistant-visible text. A visible incident artifact remains an explicit profile-confined file effect, never hook state.

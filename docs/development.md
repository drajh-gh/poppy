...
For the Housekeeping single current-task fast path, source and deterministic contract checks may support only the structural claim that eligible requests expose title-only `done` semantics before skill loading, co-load Root and Housekeeping in one bounded read, and use one composed effect call without a preliminary native task listing. A fresh trace for the exact current-task prompt is required before claiming a model realizes that structure. Measured wall-clock latency remains unverified unless a separately approved evaluation satisfies the paired performance contract above; do not convert the structural claim into a universal seconds-level SLA.

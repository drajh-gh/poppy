...
- Resource state is resolved proportionately: harmless dirt and refreshable staleness stay quiet, exact source alternatives are pinned once, unknown or user-owned state is protected, and task-owned temporary resources are retired only with supported ownership, preservation, authority, and read-back. Plugin upgrades keep in-use task-loaded predecessor paths readable.
...
- Root may offer once per completion episode to mark the current task done only after the substantive owner has fully evidenced completion and no gate remains. The offer is stateless, performs no task inspection or mutation, and grants no authority; Housekeeping acts only after an explicit user lifecycle command. Housekeeping owns Codex task lifecycle metadata and workspace organization, consumes evidenced dispositions from substantive owners, and never manufactures project completion from inactivity or title state.
...
Poppy has no cockpit, MCP server, telemetry, recurring automation, universal capability graph, daemon or persistent runtime, schema platform, persistent execution ledger, project adapter, generated repository instructions, fixed quality score, permanent agent team, or mandatory orchestration choreography. Its optional Housekeeping hooks are reviewed, deterministic, bounded, transcript-free, network-free, and stateless. Scribe declares no private persistence in this release.

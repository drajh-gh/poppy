...
- Fifth remediation evidence: candidate `50706230e02ace2f60137dfd0586ff3bad844474` passed deterministic, plugin, skill, owned-process, and 31-route forward tests but was blocked for empty plan/preflight/postflight evidence, combining/format-character trigger bypasses, unresolved or cross-project identifiers, non-mutating execution, failed workers under PASS, unstaffed fresh assessment nodes, and mutation under non-execution dispositions. The successor requires non-empty evidence contracts, Unicode category-aware alias boundaries, one normalized exact project key for substantive work, mutating-only authorized execution, verdict-to-worker-outcome consistency, a separately planned worker for every selected `fresh-worker` node, and `execute-graph` for authorized mutation.
- Sixth remediation evidence: candidate `b6124090398d5231ce07f9216ab90b6166fbeaba` passed deterministic gates and 91 of 96 forward checks but was blocked for self-asserted evaluator independence below R2, contradictory delegation budgets, canonically equivalent authority overlap, false project identity on truthful safe stops, ungrounded current-turn mutation authority, Delivery review-stage bypass, and untyped assessor inputs/outputs. The successor binds every evaluator identity and evidence, normalizes authority text with Unicode NFKC, makes depth/active/allowance budgets internally consistent, permits only the `unresolved` sentinel on a stopped substantive path, requires literal explicit current-turn authorization for each exact action, forces Delivery through separate fresh Functional QA and Final Assurance, and derives worker input/output artifacts from the selected typed graph edges.
- Seventh remediation evidence: candidate `008a65e50cbb303589ce01a754ac9f7e85350387` passed deterministic gates and 157 of 164 forward checks but was blocked for quoted, conditional, explanatory, or negated pseudo-authorization; unbound ordinary-worker evidence; delivery effects after failed gates; noncanonical unresolved sentinels; parallel or concurrent Delivery assessments; root/worker identity collapse; unbound evaluator verdicts; and boolean/float values accepted as integers. The successor accepts current-turn mutation authority only as one standalone exact grant, binds every delegated node and evaluator verdict to worker evidence, requires both Delivery gates to pass before Final Assurance continuation, authorized execution, or effects, enforces canonical `unresolved`, forbids Delivery bypass/concurrency, separates worker/root identities, and uses type-strict schema, depth, and allowance validation.
- Bounded final hardening: candidate `d0fc79f185b6d7ce6981e348b337ea261fcc6924` passed the established deterministic, plugin, skill, compile, and owned-process gates. A bounded malformed-packet check found nested wrong-type values could raise exceptions. The release wrapper fails those packets closed as `INVALID`; further exploratory adversarial expansion is deferred to evidence from real use.

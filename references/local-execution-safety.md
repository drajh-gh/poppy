...
1. Assert that the nominated and observed repository roots are syntactically absolute and have the same safe lexical identity. Reject relative, drive-relative, parent-traversing, cross-platform, device-namespace, extended-length, or alternate-data-stream aliases without resolving the filesystem. On absence, invalidity, or mismatch, stop repository-dependent work and report the affected coverage as Gray with the observed state.
...
3. Declare the complete dependency and prerequisite check set nominated by the project adapter, then record exactly one categorized result for each declaration. Require every nominated result to pass regardless of its required flag. Reject omissions, extras, duplicates, category changes, any non-passing result, inconsistent state, and evidence of a partial mutation; record a repair path rather than assuming rollback.
...
5. After interruption, emit a complete resume packet containing branch and revision, changed files, last passed gate, the exact process IDs from the authoritative owned-process ledger, a non-empty blocker, remaining external-write gates, and an exact resume instruction.

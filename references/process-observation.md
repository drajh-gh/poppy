...
Course-correction observations are working context. Do not create an attempt log, receipt, score, graph, background task, hook, watcher, telemetry stream, automation, tracker item, or memory entry. A completed supported outcome may later produce a durable lesson through Poppy Learn and, for an actual memory write, Poppy Context's separate authority and provenance gate.

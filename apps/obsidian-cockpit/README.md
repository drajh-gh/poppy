...
Multiple configured vaults share one localhost bridge. The Obsidian client derives the loopback endpoint from the packaged `bridge.json`, so health, API, SSE, and spawned service all use the same configured port. Each startup child carries an opaque instance token, while the Python service claims ownership by binding that port before opening the shared runtime. The active owner and temporary startup contenders are tracked separately, so a losing child's exit cannot clear the owner handle after a transient health failure. The plugin terminates losing or timed-out children before returning. Only the vault instance that owns the active child may stop it on unload. Another active vault can then claim a fresh singleton after its next health or SSE retry.
